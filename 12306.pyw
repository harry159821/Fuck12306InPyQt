...
# -*- coding:utf-8 -*-
__author__ = 'HeiYue'
__mail__   = 'harry159821@gmail.com'
__QQ__     = '984405219'
...
class BlobReply(QtNetwork.QNetworkReply):
    def __init__(self, url, parent=None):
        super(BlobReply, self).__init__(parent)
    ...
    def readAll(self):
        QtNetwork.QNetworkReply.readAll(self)

    def readData(self,data,maxSize):
        return None

    # def error(QNetworkReply.NetworkError)),
...
class NetworkAccessManager(QtNetwork.QNetworkAccessManager):
    def __init__(self,parent=None):
        ...
        self.parent = parent
        self.sslErrors.connect(self.handleSslErrorsFun)
        self._buffer = {}
        self.loginWindows = parent

    # def handleSslErrorsFun(self, reply, errors):
    def handleSslErrorsFun(self, errorsList):
        # print "SSL Error:",self.sender().url().toString()
        # 无视 HTTPS 证书错误
        self.sender().ignoreSslErrors()

    def createRequest(self,op,request,device):
        url = request.url().toString()
        self.parent.setWindowTitle(url)
        reply = None
        if "ad.12306.cn" in url or "close_show_citys.jpg" in url\
            or "https://kyfw.12306.cn/otn/resources/js/framework/audio/message.wav" in url:
            # QUrl(QString("forbidden://localhost/")
            reply = BlobReply(request.url(),self)
        else:
            reply = QtNetwork.QNetworkAccessManager.createRequest(self, op, request, device)
            reply.sslErrors.connect(self.handleSslErrorsFun)

        # https://kyfw.12306.cn/otn/passcodeNew/getPassCodeNew?module=login&rand=sjrand&0.8641316730063409
        if "getPassCodeNew" in url:
    ...
    def onReadReady(self):
        ...
        if "getPassCodeNew" not in url:
            return
        #因为加载顺序是异步的，所以不能使用直接缓存，最后直接在 finish 里面使用
        #因为每次的 readReady 可能来自不能的文件，通过文件来源进行键值对的缓存
        size = reply.size()
    ...
    def onFinish(self):
        ...
        if not url in self._buffer:
            return
        f = QtCore.QFile(QtCore.QString('code.jpg'))
        ok = f.open(QtCore.QFile.WriteOnly)
        if not ok:  
            print u'打开文件失败:%s' % (file_fullname)
            return
        f.write(self._buffer[url])
        f.close()
        if not self.loginWindows.isLogin:
            self.loginWindows.anlysisCode(self._buffer[url])

        self._buffer.pop(url)


class LoginWindows(QtGui.QMainWindow):
    def __init__(self,parent=None):
        ...
        self.weight = QtGui.QWidget()
        self.weight.setLayout(self.mainLayout)
        self.setCentralWidget(self.weight)

        self.font = QtGui.QFont(u"微软雅黑",24)
        self.button = QtGui.QPushButton("Push Me",self)
        self.timeEdit = QtGui.QSpinBox(self)
        self.timeEdit.setMaximum(100000)
        self.timeEdit.setValue(800)
        self.label = QtGui.QLabel("",self)
        self.result = QtGui.QLabel("",self)

        self.fromStationEdit = QtGui.QLineEdit(u'赣州',self)
        self.toStationEdit = QtGui.QLineEdit(u'北京',self)
        self.fromStationEdit.setMaximumWidth(150)
        self.toStationEdit.setMaximumWidth(150)

        self.result.setFont(self.font)
        self.button.setFont(self.font)
        self.timeEdit.setFont(self.font)
        self.fromStationEdit.setFont(self.font)
        self.toStationEdit.setFont(self.font)
        self.timeEdit.setSizePolicy(
            QtGui.QSizePolicy(QtGui.QSizePolicy.Minimum, QtGui.QSizePolicy.Minimum))
        self.timeEdit.setMaximumHeight(70)
        self.label.setFont(self.font)
        self.label.setMaximumHeight(80)

        self.hLayout.addWidget(self.button)
        self.hLayout.addWidget(self.timeEdit)
        self.hLayout.addWidget(self.fromStationEdit)
        self.hLayout.addWidget(self.toStationEdit)
        self.hLayout.addSpacing(0)
        self.hLayout.addWidget(self.result)
        self.hLayout.addItem(QtGui.QSpacerItem(208, 78, 
        ...
        self.networkManager = NetworkAccessManager(self)
        self.webView.page().setNetworkAccessManager(self.networkManager)
        self.webView.page().setForwardUnsupportedContent(True)

        self.timer = QtCore.QTimer()
        self.timer.timeout.connect(self.timeOutFun)
        self.timer.setInterval(int(self.timeEdit.text()))
        ...
        if os.path.isfile("cookie"):
            with open('cookie') as f:
                cookieString = f.read()
                if cookieString and self.checkCookie(cookieString):
                    self.webView.setUrl(QtCore.QUrl("https://kyfw.12306.cn/otn/login/checkUser"))
                else:
                    self.webView.setUrl(QtCore.QUrl("https://kyfw.12306.cn/otn/login/init"))
        else:
            self.webView.setUrl(QtCore.QUrl("https://kyfw.12306.cn/otn/login/init"))

        cookieString = cookieString.replace(" ","")
        self.cookieList = cookieString.split(";")
    ...
    def checkCookie(self,cookie):
        ...
        req = urllib2.Request("http://kyfw.12306.cn/otn/login/checkUser",headers=headers)
    ...
    def center(self,screenNum=0):
        screen = QtGui.QApplication.desktop().availableGeometry(screenNum)
        size = self.geometry()
    ...
    def timeOutFun(self):
        ...
        frame = self.webView.page().mainFrame()
        document = frame.documentElement()
        res = document.evaluateJavaScript('$("#query_ticket").click()'+ ';null')
        res = document.evaluateJavaScript('''$("#query_ticket").removeClass('btn92s btn-disable').addClass('btn92s')'''+ ';null')
        ...
        body = frame.findFirstElement("body")
        body.setAttribute("onunload","myFunction()")
        self.webView.settings().clearMemoryCaches()
        self.webView.history().clear()

        if self.loop:
            self.label.setText(str(self.timeValue)+u'ms等待ing')
            self.timeValue = self.timeEdit.value()
            QtCore.QTimer.singleShot(self.timeValue,self.timeOutFun)

    def waittingForBox(self):
        frame = self.webView.page().mainFrame()
        document = frame.documentElement()
        # 等待 LoadingBox 关闭
        i = -1
        while not self.isLoadingBoxNull():
            QtGui.QApplication.processEvents()
            time.sleep(0.01)
            i += 1
            self.label.setText(u'查询中:'+str(150-i))
            if i > 150:
                # print time.time(),"stop timeout page"
                self.webView.stop()
                document.evaluateJavaScript('''$("#query_ticket").removeClass('btn92s btn-disable').addClass('btn92s')'''+ ';null')
    ...
    def clickImage(self):
        # frame = self.webView.page().mainFrame()
        # document = frame.documentElement()
        # img = document.findFirst('img[class="touclick-image"]')
        # geometry = img.geometry()

        # l = self.code.split(',')
        # for pos in [l[i:i+2] for i in xrange(0, len(l), 2)]:
        #     x = str(int(pos[0])+geometry.topLeft().x())
        #     y = str(int(pos[1])+geometry.topLeft().y())
        #     print x,y
        #     document.evaluateJavaScript('document.elementFromPoint('
        #         +x+','+y+').click();null')

        cookie = ';'.join([str(c.name()+'='+c.value()) for c in self.networkManager.cookieJar().allCookies()])
        request = QtNetwork.QNetworkRequest(QtCore.QUrl("https://kyfw.12306.cn/otn/passcodeNew/checkRandCodeAnsyn"))
        request.setRawHeader('User-agent', 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US)')
        request.setRawHeader("Accept-Language","en-us,en;q=0.5")
        request.setRawHeader("Accept-Charset","ISO-8859-1,utf-8;q=0.7,*;q=0.7")
        request.setRawHeader("Connection","keep-alive")
        request.setRawHeader("Content-Type","application/x-www-form-urlencoded")
        request.setRawHeader("Cookie",cookie)
        post_data = QtCore.QByteArray()
        post_data.append("randCode="+self.code+"&")
        post_data.append("rand=sjrand&")
        post_data.append("_json_att=&")
        i = 370
        while i:
            QtGui.QApplication.processEvents()
            time.sleep(0.01)
            i -= 1
            self.label.setText(u'等待跳转:'+str(i))
        self.webView.load(request,QtNetwork.QNetworkAccessManager.PostOperation,post_data)

        # loginUrl = 'https://kyfw.12306.cn/otn/login/loginAysnSuggest'
        # loginData = {
        #     'loginUserDTO.user_name':open('username').read(),
        #     'userDTO.password':open('password').read(),
        #     'randCode':code
        # }

    def anlysisCode(self,codeData):
        self.button.setText(u"自动识别验证码中")
        base64data = base64.b64encode(codeData)
        data = {
            "method":"upload",
            "ts":str(time.time()).replace('.','')+'0',
            "guid":"1f1c4d73eaf9fde3eafab69d70c178cf",
            "file":base64data,
            "code_type":"",
        }
        headers = {
            'User-agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.104 Safari/537.36 Core/1.53.2039.400 QQBrowser/9.5.10102.400',
            "Referer":"https://kyfw.12306.cn/",
        }
        req = urllib2.Request("http://12306.qq.com/api/get_code",data=urllib.urlencode(data),headers=headers)
        res = urllib2.urlopen(req).read()
        data = json.loads(res)
        ...
        if data['ret'] == 1:
            self.code = urllib.unquote(data['data'])
            print self.code
            self.button.setText(u"自动识别成功3.7秒跳转")
            QtCore.QTimer.singleShot(10,self.clickImage)
            return self.code
        else:
            self.button.setText(u"识别失败")
            QtCore.QTimer.singleShot(500,self.refreshCode)
            return -1
    ...
    def checkTicket(self):
        levelDict = [
            u'商务座',u'特等座',u'一等座',u'二等座',u'高级软卧',u'软卧',
            u'硬卧',u'软座',u'硬座',u'无座',u'其他',u'备注']
        filterDict = [
            u'商务座',u'特等座',u'一等座',u'二等座',u'高级软卧',u'软卧',u'其他',u'备注',u'无座',u'硬座'
        ]
        def inFilterDict(level):
            for item in filterDict:
                if levelDict[level] == item:
                    return True
            return False
        frame = self.webView.page().mainFrame()
        document = frame.documentElement()
        table = document.findFirst('tbody[id="queryLeftTable"]')
        for tr in table.findAll('tr'):
            tdList = tr.findAll('td[align="center"]')
            if not len(tdList):
                # 过滤无用的列表
                continue
            level = -1
            for td in tdList:
                # 过滤不要的座位
                level += 1
                if inFilterDict(level):
                    continue
                ticket = td.toPlainText()
                if u"--" in ticket or u"无" in ticket or u"*" in ticket:
                    continue
                elif "K4203" in td.attribute("id"):
                    continue
                elif "K1454" in td.attribute("id"):
                    continue
                else:
                    self.loop = False
                    self.result.setText(u"车次:"+td.attribute("id")\
                        +u""+levelDict[level] +u"票数:"+ticket)
                    self.button.setText(u"有票啦")
                    QtGui.QApplication.processEvents()

                    # Auto Get Ticket
                    # 自动点击预定
                    tr.evaluateJavaScript(tr.findFirst(u'a[class="btn72"]').attribute('onclick'))
                    # tr.evaluateJavaScript('$(".btn72").click();null')
                    self.showMaximized()
                    self.center(0)
                    self.activateWindow()
                    ...
                    return True
        self.result.setText(u"么有车呢")
        return False

    def checkHailWay(self):
        frame = self.webView.page().mainFrame()
        document = frame.documentElement()
        for bgc in document.findAll('tr[class="bgc"]'):
            level = 0
            for td in (bgc.findAll('td[align="center"]')):
                level += 1
                # 不要商务座
                if level < 2:
                    continue
                div = td.findFirst("div")
                if div.isNull():
                    continue
                if div.toPlainText() == u"无":
                    continue
                if div.toPlainText() == u"*":
                    continue
                else:
                    self.loop = False
                    self.result.setText(u"车次:"+td.attribute("id")+u"票数:"+div.toPlainText())
                    self.button.setText(u"有票啦")
                    QtGui.QApplication.processEvents()
                    bgc.evaluateJavaScript('$(".btn72").click()')
                    self.showMaximized()
                    self.center(0)
                    self.activateWindow()
                    winsound.Beep(1200,1000)
                    return True
        self.result.setText(u"么有高铁呢")
        return False

    def buttonClicked(self):
        self.loop = False if self.loop else True
        if self.loop:
            self.button.setText(u"刷新中")
            self.timeOutFun()
        else:
            self.button.setText(u"停止刷新中")
        self.checkTicket()

    def typeTicket(self):
        frame = self.webView.page().mainFrame()
        document = frame.documentElement()
        startStation = document.findFirst('input[id="fromStation"]')
        EndStation = document.findFirst('input[id="toStation"]')
        queryButton = document.findFirst('a[id=query_ticket]')
        sName = unicode(self.fromStationEdit.text())
        tName = unicode(self.toStationEdit.text())
        startStation.setAttribute('value',findStation(sName))
        EndStation.setAttribute('value',findStation(tName))
        document.evaluateJavaScript(u'$("#fromStationText").val("'+sName+'")'+ ';null')
        document.evaluateJavaScript(u'$("#toStationText").val("'+tName+'")'+ ';null')
        document.evaluateJavaScript(u'$("#train_date").val("2017-02-04")'+ ';null')
        document.evaluateJavaScript('$("#query_ticket").click()'+ ';null')
        document.evaluateJavaScript('$("#query_ticket").click()'+ ';null')

        self.waittingForBox()

    def getLoadingUrl(self):
        if self.webView:
    ...
    def setWebView(self):
        '''获取WebView对象'''
        self.webView = QtWebKit.QWebView(self) # 生成 webview 对象
    ...
    def loadStarted(self):
        print "LoadStarted:"+self.getLoadingUrl()

    def loadProgress(self,progress):
        self.label.setText(u'LoadProgress:'+str(progress))

    def loadFinished(self):
        frame = self.webView.page().mainFrame()
        document = frame.documentElement()
        url = self.getLoadingUrl()

        if 'checkRandCodeAnsyn' in url:
            cookie = ';'.join([str(c.name()+'='+c.value()) for c in self.networkManager.cookieJar().allCookies()])
            request = QtNetwork.QNetworkRequest(QtCore.QUrl("https://kyfw.12306.cn/otn/login/loginAysnSuggest"))
            request.setRawHeader('User-agent', 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US)')
            request.setRawHeader("Accept-Language","en-us,en;q=0.5")
            request.setRawHeader("Accept-Charset","ISO-8859-1,utf-8;q=0.7,*;q=0.7")
            request.setRawHeader("Connection","keep-alive")
            ...
            request.setRawHeader("Cookie",cookie)
            post_data = QtCore.QByteArray()
            post_data.append('loginUserDTO.user_name='+open('username').read()+"&")
            post_data.append('userDTO.password='+open('password').read()+"&")
            post_data.append('randCode='+self.code+"&")
            self.webView.load(request,QtNetwork.QNetworkAccessManager.PostOperation,post_data)

        if 'loginAysnSuggest' in url:
            self.webView.setUrl(QtCore.QUrl("https://kyfw.12306.cn/otn/index/initMy12306"))

        if "https://kyfw.12306.cn/otn/login/init" in url:
            # 自动填写账号密码
            document.evaluateJavaScript('$("input#username").val("'+self.username+'")' + ";null")
            document.evaluateJavaScript('$("input#password").val("'+self.password+'")' + ";null")
        ...
        if url == 'https://kyfw.12306.cn/otn/leftTicket/init':
            ...
            if self.loop:
                # 如果 Loop 就启动循环刷票定时器
                self.checkTicket()
                QtCore.QTimer.singleShot(2000,self.timeOutFun)            
                self.button.setText(u"刷新中")

        if "https://kyfw.12306.cn/otn/confirmPassenger/initDc" in url:
            # 在预订页面自动选择第一个联系人并点击订购
            document.evaluateJavaScript('(($("label")).prev())[0].click()' + ";null")
            document.evaluateJavaScript('$(".btn92s").click()'+ ";null")

        if "https://kyfw.12306.cn/otn/index/initMy12306" in url:
            # 登陆到个人信息页面成功后更新 Cookie 文件
            cookie = document.evaluateJavaScript('document.cookie').toString()
            with open('cookie','w') as f:
                f.write(cookie)
            if not self.isLogin:
                self.isLogin = True
                self.webView.setUrl(QtCore.QUrl("https://kyfw.12306.cn/otn/leftTicket/init"))

        if "https://kyfw.12306.cn/otn/login/checkUser" in url:
            self.initTimes += 1

            if self.initTimes == 0:
                # 第一次加载页面生成 Web Document 对象 并清空无用 Cookie
                self.deleteAllCookies()
                # 强制写入 Cookie 到浏览器对象
                for cookie in self.cookieList:
                    name = cookie[:cookie.index("=")]
                    value = cookie[cookie.index("=")+1:]
                    document.evaluateJavaScript('document.cookie = "'+ cookie +';path=/";null')
                # 重载页面
                self.webView.reload()
        ...
        print "LoadFinished:"+self.getLoadingUrl()
